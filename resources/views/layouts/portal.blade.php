...
<body class="bg-[#F3F7FB] text-[#111827] antialiased">
    ...
    <header class="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 gap-4">
            ...
            <div class="flex items-center gap-3">
                <button id="portal-sidebar-toggle" aria-label="القائمة" class="lg:hidden p-2 rounded-xl text-gray-500 hover:bg-gray-100 transition-colors">
            ...
            <div class="flex items-center gap-3">
                <div class="hidden sm:flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style="background:#253B5B">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm font-medium px-3 py-1.5 rounded-xl transition-colors hover:bg-red-50" style="color:#EF4444">
                        خروج
                    </button>
                </form>
            </div>
    ...
    {{-- Mobile overlay --}}
    <div id="portal-overlay" class="fixed inset-0 z-30 bg-black/40 hidden lg:hidden" aria-hidden="true"></div>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex gap-7">

        {{-- ── Sidebar (first child = RIGHT in RTL) ──────────────────────── --}}
        <aside id="portal-sidebar" class="fixed top-16 right-0 h-[calc(100vh-4rem)] w-64 z-30 translate-x-full overflow-y-auto
            ...
            <nav class="p-3 lg:bg-white lg:rounded-3xl lg:border lg:border-gray-100 lg:shadow-sm">
                ...
                <a href="{{ route($item['route']) }}" class="flex items-center gap-3 px-3 py-3 rounded-2xl text-sm font-medium transition-all duration-200 mb-1
                          {{ $active ? 'text-white shadow-sm' : 'text-gray-600 hover:bg-[#EAF2FA] hover:text-[#253B5B]' }}" style="{{ $active ? 'background:#253B5B' : '' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="{{ $active ? 'white' : '#6B7280' }}">
                        {!! $item['icon'] !!}
                    </svg>
