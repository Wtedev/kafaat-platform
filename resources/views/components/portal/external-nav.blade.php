...
<div {{ $attributes->class($variant === 'sidebar' ? 'space-y-2' : 'flex shrink-0 items-center gap-1 sm:gap-2') }}>
    ...
    <a href="{{ url('/admin') }}" class="{{ $linkClass }}" aria-label="لوحة الإدارة">
        ...
        <span class="sr-only sm:not-sr-only sm:ms-1 sm:inline sm:text-xs sm:font-medium">لوحة الإدارة</span>
    </a>
    @endif
    @if ($showMainSite)
    <a href="{{ route('home') }}" class="{{ $linkClass }}" aria-label="العودة للموقع الرئيسي">
        ...
        <span class="sr-only sm:not-sr-only sm:ms-1 sm:inline sm:text-xs sm:font-medium">الموقع</span>
    </a>
    @endif
</div>
@endif
