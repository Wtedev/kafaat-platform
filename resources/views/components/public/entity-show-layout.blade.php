...
<nav class="mb-5" aria-label="مسار التنقل">
    <a
        href="{{ $backHref }}"
        class="inline-flex items-center gap-2 rounded-lg text-sm font-medium transition-colors hover:text-[#1e3a5f]"
        style="color:#335483"
    >
...
<div @class([
    'grid gap-6',
    'grid-cols-1',
    $hasSidebar ? 'md:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]' : '',
])>
    @if ($hasSidebar)
    ...
    <div @class([
        'order-1 min-w-0 md:order-none',
        $hasSidebar ? 'md:col-start-2 md:row-start-1' : '',
    ])>
