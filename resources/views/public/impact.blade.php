...
<head>
    ...
    <style>
        ...
        .gradient-text {
            background: linear-gradient(135deg, #002a30 0%, #004a54 50%, #00616f 100%);
...
<body class="bg-[#F7FAFC] text-[#111827] antialiased font-sans">
    ...
    {{-- 1. HERO                                                                 --}}
    {{-- ═══════════════════════════════════════════════════════════════════════ --}}
    <section class="relative overflow-hidden" style="background: linear-gradient(135deg, #002a30 0%, #004a54 40%, #00616f 100%)">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            ...
            <div class="absolute bottom-0 left-0 w-[600px] h-[600px] rounded-full" style="background: radial-gradient(circle, rgba(0,97,111,0.10) 0%, transparent 65%);
                        transform: translate(-30%,30%)"></div>
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 text-center">
            ...
            <div class="reveal-fade inline-flex items-center gap-2.5 px-5 py-2 rounded-2xl border mb-8" style="background:rgba(0,97,111,0.12); border-color:rgba(0,97,111,0.30); color:#00616f">
                <span class="w-2 h-2 rounded-full flex-shrink-0 animate-pulse" style="background:#00616f"></span>
            ...
            <div class="reveal-fade mb-5" style="transition-delay:0.08s">
                <img
                    ...
                    alt="عام الأثر"
                    class="mx-auto h-16 sm:h-20 md:h-24 w-auto"
                />
            </div>
            ...
            <div class="reveal-fade flex flex-wrap items-center justify-center gap-4" style="transition-delay:0.32s">
                <a href="{{ route('public.programs.index') }}" class="inline-flex items-center gap-3 px-8 py-4 rounded-2xl font-semibold text-white text-base
                          shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl" style="background: linear-gradient(135deg, #00616f 0%, #004a54 100%)">
                    استكشف البرامج
    ...
    <section class="py-24 bg-white">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-16 reveal-fade">
                <p class="text-sm font-semibold uppercase tracking-widest mb-3" style="color:#00616f">التحول المنشود</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                $b = config('brand');
                $iconPalette = [
                    ['color' => $b['primary'], 'bg' => 'rgba(51,84,131,0.08)'],
                    ['color' => $b['secondary'], 'bg' => 'rgba(26,147,153,0.08)'],
                    ['color' => $b['accent'], 'bg' => 'rgba(251,187,46,0.12)'],
                ];
                $shifts = [
                [
                'from' => 'قياس الجهد',
                ...
                <div class="transformation-item reveal-fade rounded-3xl border border-gray-100 p-8 text-right" style="background:#F3F7FB; transition-delay:{{ $shift['delay'] }}">
                    ...
                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center mb-6" style="background:{{ $tone['bg'] }}">
                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="{{ $tone['color'] }}">
                            {!! $shift['icon'] !!}
                        </svg>
                    </div>
    ...
    <section class="py-24" style="background:#F3F7FB">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-16 reveal-fade">
                <p class="text-sm font-semibold uppercase tracking-widest mb-3" style="color:#00616f">الركائز الأربع</p>
                <h2 class="text-3xl sm:text-4xl font-bold" style="color:#111827">أركان الأثر</h2>
            </div>

            @php
            $pillarPalette = [
                ['color' => $b['primary'], 'glow' => 'rgba(51,84,131,0.10)', 'ring' => 'rgba(51,84,131,0.22)'],
                ['color' => $b['secondary'], 'glow' => 'rgba(26,147,153,0.10)', 'ring' => 'rgba(26,147,153,0.22)'],
                ['color' => $b['accent'], 'glow' => 'rgba(251,187,46,0.14)', 'ring' => 'rgba(251,187,46,0.30)'],
                ['color' => $b['danger'], 'glow' => 'rgba(236,96,86,0.10)', 'ring' => 'rgba(236,96,86,0.22)'],
            ];
            $pillars = [
            [
            'icon' => '
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                @php $tone = $pillarPalette[$i]; @endphp
                <div class="impact-card reveal-fade group relative bg-white rounded-3xl p-8 text-right border border-gray-100 overflow-hidden cursor-default" style="transition-delay:{{ $pillar['delay'] }}">
                    ...
                    {{-- Hover glow --}}
                    <div class="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style="background: radial-gradient(ellipse at 50% -10%, {{ $tone['glow'] }}, transparent 70%)"></div>

                    {{-- Bottom accent --}}
                    <div class="absolute bottom-0 right-0 left-0 h-[3px] rounded-b-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" style="background: linear-gradient(to left, {{ $tone['color'] }}, transparent)"></div>

                    {{-- Icon --}}
                    <div class="relative w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-transform duration-300 group-hover:scale-110" style="background:{{ $tone['glow'] }}; box-shadow: 0 0 0 1px {{ $tone['ring'] }}">
                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="{{ $tone['color'] }}">
                            {!! $pillar['icon'] !!}
                        </svg>
                    </div>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div class="reveal-fade relative">
                ...
                <div class="absolute inset-0 flex items-start justify-center select-none pointer-events-none" aria-hidden="true">
                    <span class="font-black" style="font-size:16rem; color:rgba(0,97,111,0.04); line-height:0.65">"</span>
                </div>
                ...
                <div class="mt-8 w-20 h-1 rounded-full mx-auto" style="background: linear-gradient(to left, #00616f, #007a88)"></div>
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════ --}}
    <section class="py-24" style="background: linear-gradient(180deg, #F3F7FB 0%, #e9eff6 100%)">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row items-center gap-14">

                {{-- Text (RIGHT in RTL) --}}
                <div class="w-full lg:w-1/2 text-right reveal-fade">
                    <p class="text-sm font-semibold uppercase tracking-widest mb-3" style="color:#00616f">الإطار الاستراتيجي</p>
                    ...
                    <div class="flex flex-wrap gap-3">
                        <a href="{{ route('public.programs.index') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-semibold text-white
                                  shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5" style="background:#335483">
                        ...
                        <a href="{{ route('public.paths.index') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-semibold border-2
                                  hover:bg-[#e9eff6] transition-all duration-200" style="color:#335483; border-color:#c5d4e4">
                            المسارات التدريبية
                        </a>
                    </div>
                </div>
                ...
                <div class="w-full lg:w-1/2 reveal-fade" style="transition-delay:0.15s">
                    @php
                    $stratPillars = [
                    ['label' => 'تعظيم الأثر التنموي',      'bg' => 'rgba(51,84,131,0.08)',  'stroke' => $b['primary'], 'path' => 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6'],
                    ['label' => 'التحول الرقمي',             'bg' => 'rgba(26,147,153,0.08)', 'stroke' => $b['secondary'], 'path' => 'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
                    ['label' => 'الاستدامة المالية',         'bg' => 'rgba(251,187,46,0.12)', 'stroke' => $b['accent'], 'path' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                    ['label' => 'تطوير رأس المال البشري',   'bg' => 'rgba(236,96,86,0.08)',  'stroke' => $b['danger'], 'path' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],
                    ];
                    @endphp
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm text-right
                            ...
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-3" style="background:{{ $sp['bg'] }}">
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center reveal-fade">
            ...
            <div class="flex flex-wrap items-center justify-center gap-4">
                ...
                <a href="{{ route('public.volunteering.index') }}" class="px-8 py-4 rounded-2xl text-base font-semibold border-2 bg-white
                          hover:bg-[#e9eff6] transition-all duration-200 hover:-translate-y-0.5" style="color:#335483; border-color:#c5d4e4">
                    الفرص التطوعية
                </a>
            </div>
        </div>
