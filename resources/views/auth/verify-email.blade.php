...
<form method="POST" action="{{ route('verification.verify') }}">
    ...
    <input type="text" id="code" name="code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" required autofocus placeholder="000000" class="w-full mb-4 py-3 rounded-xl border border-gray-200 text-center text-2xl font-bold tracking-[0.5em] text-gray-900 focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none" dir="ltr">
...
<form method="POST" action="{{ route('verification.send') }}" class="mt-3">
    @csrf
    <button type="submit" class="w-full py-2.5 rounded-xl border border-gray-200 text-gray-600 font-medium text-sm hover:bg-gray-50 transition">
        إعادة إرسال الرمز
    </button>
</form>
...
<form method="POST" action="{{ route('logout') }}" class="mt-3">
    @csrf
    <button type="submit" class="w-full py-2.5 rounded-xl text-gray-500 font-medium text-sm hover:text-gray-700 transition">
        تسجيل الخروج
    </button>
</form>
