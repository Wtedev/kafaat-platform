...
<section class="mb-6 flex flex-wrap items-start justify-between gap-3 text-right">
    <div>
        ...
        <p class="mt-2 max-w-2xl text-sm leading-relaxed text-gray-600">صندوق موحّد لكل ما يخص حسابك — التسجيل، القبول، التحديثات، والشهادات.</p>
    </div>
    <a href="{{ route('portal.notifications.settings') }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 shadow-sm transition hover:border-[#335483]/30 hover:text-[#335483]">
...
@include('portal.partials.inbox-notifications-panel', [
    ...
    'panelTitle' => 'صندوق التنبيهات',
    'panelSubtitle' => 'غير مقروء: '.en_num($unreadCount),
])
@endsection
