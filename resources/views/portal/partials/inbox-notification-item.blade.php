    ...
    <div @class([
        ...
        <div class="min-w-0 flex-1 text-right">
            ...
            @if ($whatsappUrl)
                <div class="mt-2.5">
                    <a
                        ...
                        class="inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition hover:opacity-95"
            ...
            <div @class([
                ...
                @if ($inboxOpenUrl)
                    <a href="{{ $inboxOpenUrl }}" @if(\App\Filament\Support\InboxNotificationRecordActions::publicUrl($n) === $inboxOpenUrl) target="_blank" rel="noopener noreferrer" @endif class="inline-flex items-center rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-[#335483] ring-1 ring-[#c5d4e4] transition hover:bg-[#e9eff6]">
                        {{ $inboxOpenLabel }}
                    </a>
                @endif
                ...
                @if ($isUnread)
                    <form method="POST" action="{{ route('portal.notifications.read', $n) }}">
                        @csrf
                        <button type="submit" class="rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-gray-600 ring-1 ring-gray-200 transition hover:bg-gray-50">تعليم كمقروء</button>
                    </form>
                @endif
            </div>
            @endif
