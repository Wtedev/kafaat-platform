...
<article class="portal-dash-card group flex min-w-[17.5rem] max-w-[17.5rem] flex-none snap-start flex-col sm:min-w-[19rem] sm:max-w-[19rem]">
    @if ($imageUrl)
    <div class="relative h-28 w-full shrink-0 overflow-hidden bg-brand-light">
        <img src="{{ $imageUrl }}" alt="" class="h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" loading="lazy" decoding="async" />
    ...
    <div class="flex flex-1 flex-col p-4 sm:p-5">
        ...
        <h3 class="mb-2 text-right text-base font-bold leading-snug text-slate-900">{{ $activity['title'] }}</h3>
        @if ($meta)
        <p class="mb-3 text-right text-xs text-slate-500">{{ $meta }}</p>
        @endif
